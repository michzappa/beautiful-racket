...
;; apl is the starting state of the bf program
(define (fold-funcs apl bf-funcs)
  (for/fold ([current-apl apl]) ;; the state accumulator
            ([bf-func (in-list bf-funcs)]) ;; calls each function in this list with the state accumulator
...
;; returns the symbol reference to a function so fold-funcs can apply with it
